...
# Before starting with the code snippets, show the models involved
...
# Note: in Manage those will NOT create an N+1 query because we user our Jit preloader magic to prevent them out of the box
...
# Step 5
# Fix using eager load when used in where/order statement
#
...
Contact.all.map { |x| x.email_addresses.select { |z| !z.bounced } }
...
# Note: This circumvents the N+1 detection because it does not use associations!!
...
# Step 7:
# Aggregate and max associations using Jit preloader
...
# In Manage, we use JIT preloader in multiple places to avoid simple N+1 queries
# by monkey-patching the ActiveRecord functionality
# We enable JitPreloader in the config/initializers/jit_preloader.rb initializer
# Show some examples.